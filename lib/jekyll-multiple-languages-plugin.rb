...
module Jekyll
  ...
  module MultipleLanguages
    class << self
      def should_be_excluded(static_file_or_page, exclude_paths)
        # Remove "/" from beginning of static file relative path
        if static_file_or_page.instance_variable_get(:@relative_path) != nil
          static_file_or_page_r_path = static_file_or_page.instance_variable_get(:@relative_path).dup
          if static_file_or_page_r_path
            static_file_or_page_r_path[0] = '' if static_file_or_page_r_path[0] == '/'

            exclude_paths.any? do |exclude_path|
              Pathname.new(static_file_or_page_r_path).descend do |static_file_or_page_path|
                break(true) if (Pathname.new(exclude_path) <=> static_file_or_page_path) == 0
              end
            end
          end
        end
...
# task: files that don't differ in

    if default_lang != current_lang
      static_files.delete_if { |p| MultipleLanguages.should_be_excluded(p, exclude_paths) }
      site.pages.delete_if do |page|
        MultipleLanguages.should_be_excluded(page, exclude_paths)
      end
    end
